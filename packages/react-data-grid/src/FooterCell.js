...
function simpleCellRenderer(objArgs: {column: {name: string}}): ReactElement {
  let footerText = objArgs.column.rowType === 'footer' ? objArgs.column.name : '';
  return <div className="widget-FooterCell__value">{footerText}</div>;
}
...
class FooterCell extends React.Component {
  ...
  render(): ?ReactElement {
    ...
    let className = joinClasses({
      'react-grid-FooterCell': true,
      // 'react-grid-FooterCell--resizing': this.state.resizing,
      'react-grid-FooterCell--locked': this.props.column.locked
    });
